...
impl NoiseMap {
    ...
    pub fn new() -> Self {
        ...
        noise.set_fractal_lacunarity(Some(1.8));
    ...
    pub fn sample(&self, x: u32, y: u32) -> (TerrainType, u8) {
        ...
        if noise < 0.15 {
            ...
            let fuel_load = 12.0 * (noise - 0.15) / 0.3;
            (TerrainType::Grassland, fuel_load.clamp(1.0, 18.0) as u8)
        } else if noise < 0.7 {
            let fuel_load = 18.0 * (noise - 0.45) / 0.25;
            (TerrainType::Tree, fuel_load.clamp(2.0, 10.0) as u8)
        } else {
